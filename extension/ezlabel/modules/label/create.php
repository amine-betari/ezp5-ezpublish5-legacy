...
if ( $http->hasVariable('CreateButton') ) {
    if( $http->hasVariable('Name') && 
        $http->variable('Name') != "" && 
        $http->hasVariable('LabelScope') && 
        ( $http->variable('LabelScope') == "global" ||
           $http->variable('LabelScope') == "private" ) )
    {
        $label['name'] = $http->variable('Name');
        $objectId = false;
        $privateLabel = true;
        ...
        if( $http->variable('LabelScope') == "private" )
        {
            $privateLabel = true;
        }
        else
        ...
        $label = eZLabel::createNew( $user_id, $label['name'], $privateLabel, $objectId );
    }
