{default $view_parameters            = array()
         ...
         $attributes_display_by_rules = ezini( 'RulesCustomSettings', 'Attributes', 'site.ini' )
         $roles_admitted = ezini( 'RulesCustomSettings', 'RolesAdmitted', 'site.ini' )}

    {def $current_creator=fetch( user, current_user )}
    {def $roles_id_list = $current_creator.role_id_list}
...
{def $display_attribute = false()}

{*Attribute with custom rules : check rights*}
{if $attributes_display_by_rules|contains( $attribute_identifier )}
    {foreach $roles_admitted as $role_admitted}
        {if $roles_id_list|contains( $role_admitted )}
            {set $display_attribute = true()}
            {break}
        {/if}
    {/foreach}
{else}
