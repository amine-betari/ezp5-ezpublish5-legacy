...
*/
    ...
    */
    function checkMonitorConnection()
    {
        ...
        $port = eZNetSOAPSync::getPort( $server, $path, $ini->variable( 'NetworkSettings', 'Port' ) );
        ...
        {
            $this->appendToBlock( "Could not contact $server:$port, this is needed for all network cronjobs" );
            if ( in_array( $port, array( 443, 'ssl' ) ) )
            {
                $this->appendToBlock( "You might want to try port 80 instead, see sync.ini [NetworkSettings] Port." );
                $this->appendToBlock( "Please note, with port 80 the connection is not encrypted." );
            }
            $this->setBlockHasError();
        }
        $this->commitBlock();
