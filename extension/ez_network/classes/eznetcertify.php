    ...
    */
    ...
    public function env()
    {
        ...
        if ( !$this->checkMonitorConnectionTo( $server, $port ) )
        {
            ...
            {
                $this->result['error']['critmon'] .=
                    "\n  You might want to try port 80 instead, see sync.ini [NetworkSettings] Port." .
                    "\n  Please note, with port 80 the connection is not encrypted.";
            }
        }
    ...
    protected function checkMonitorConnectionTo( $host, $port )
    {
        ...
        $port = eZNetSOAPSync::getPort( $host, $path, $port );
        $client   = new eZSOAPClient( $host, $path, $port );
