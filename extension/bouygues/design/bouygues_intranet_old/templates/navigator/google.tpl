...
<p>
     ...
     {switch match=$:item_previous|lt(0) }
       {case match=0}
      <a href={concat($page_uri,$:item_previous|gt(0)|choose('',concat($:offset_text,$:item_previous)),$:view_parameter_text,$page_uri_suffix, '?pgacc=',$page|dec())|ezurl} class="previous"><span class="text">{"Previous"|i18n("design/bouygues/navigator")}</span></a>
       {/case}
       {case match=1}
       {/case}
     {/switch}
...
<span class="other"><a href={concat($page_uri,$:view_parameter_text,$page_uri_suffix)|ezurl}>1</a></span>
...
{/section}
    ...
    {section loop=$:left_length}
        {let page_offset=sum(sub($ViewParameter:current_page,$ViewParameter:left_length),$:index)}
          <span class="other"><a href={concat($page_uri,$:page_offset|gt(0)|choose('',concat($:offset_text,mul($:page_offset,$item_limit))),$ViewParameter:view_parameter_text,$page_uri_suffix, '?pgacc=',$:page_offset|inc)|ezurl}>{$:page_offset|inc}</a></span>
        {/let}
    {/section}
...
<span class="other">...</span>
{/section}
<span class="other"><a href={concat($page_uri,$:page_count|dec|gt(0)|choose('',concat($:offset_text,mul($:page_count|dec,$item_limit))),$:view_parameter_text,$page_uri_suffix, '?pgacc=',$:page_count)|ezurl}>{$:page_count}</a></span>
{/section}
    ...
    {switch match=$:item_next|lt($item_count)}
      {case match=1}
        <a href={concat($page_uri,$:offset_text,$:item_next,$:view_parameter_text,$page_uri_suffix, '?pgacc=',$page|inc())|ezurl} class="next"><span class="text">{"Next"|i18n("design/bouygues/navigator")}</span></a>
      {/case}
      {case}
      {/case}
    {/switch}
