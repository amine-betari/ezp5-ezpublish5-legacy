...
{def $booRestriced = false()
	 $user = fetch('user','current_user')
	 $restrictedGroups =  ezini( 'restrictedGroups', 'Groups', 'bouygues.ini' )
	 $arrMenuAllow = array()
}


{foreach $user.groups as $group}
	{if is_set($restrictedGroups[$group])} 
		{set $arrMenuAllow = $arrMenuAllow|merge( $restrictedGroups[$group]|explode(';') )}
	{/if}
{/foreach}


{*$arrMenuAllow|attribute('show',1)*}
{*$restrictedGroups|attribute('show',1)*}
{*$user.groups|attribute('show',1)*}
...
{*$Menu|attribute('show',1)*}

	{*$navigation_part.identifier*}
	
	{if $user.groups|contains(ezini( 'UserSettings', 'AdminUserGroupID', 'bouygues.ini' ))}
		{set $booRestriced = true()}
	{else}	
		{if $arrMenuAllow|contains($Menu.navigationpart_identifier)}
			  {set $booRestriced = true()}
		{/if}
	{/if}
