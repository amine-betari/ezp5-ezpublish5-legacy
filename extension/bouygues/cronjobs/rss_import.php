<?php
//
// alternative au rssimport original (cronjob/rssimport.php), pour générer les objets (unique) 
// il se pase sur 2 attributs link et guid hors sur le feed de LCI les liens sont tous identiques
// ajout de fonction time pour générer la variable $md5Sum
...
function importRSSItem( $item, $rssImport, $cli, $channel )
{
    ...
    if ( $link->textContent )
    {
        $md5Sum = md5( $link->textContent.time() );
    }
