...
class eZCreateClass extends eZXMLInstallerHandler
{
    ...
    function execute( $xml )
    {
        ...
        foreach ( $classList as $class )
        {
            ...
            $alwaysAvailable        = $class->getAttribute( 'alwaysAvailable' );
            ...
            if ( $class )
            {
                ...
                switch( $classExistAction )
                {
                    case 'replace':
                    {
                        ...
                        $class->setAttribute( 'always_available', $alwaysAvailable );
            ...
            foreach ( $classAttributeList as $classAttributeNode )
            {
				...
				$attributeCategory =  $classAttributeNode->getAttribute( 'category' );
                //Update Kaliop : manage attribute description
                $attributeDescription = $classAttributeNode->getAttribute( 'description' );
                $attributeDataText2 =  $classAttributeNode->getAttribute( 'data_text2' );
                ...
                $params['category']                = $attributeCategory;
                ...
                $params['data_text2']               = $attributeDataText2;
...
//     }
    ...
    function addClassAttribute( $class, $params )
    {
        ...
        $attrCategory  = isset( $params['category'] )       ? $params['category'] : '';
        ...
        //Update Kaliop : set attribute description
        if($params['description'])
        {
            $newAttribute->setDescription( $params['description'] );
        }
    ...
    function updateClassAttribute( $class, $params )
    {
        ...
        $classAttribute->setAttribute( 'category', $params['category'] );
        ...
        if($params['description'])
        {
            $classAttribute->setDescription( $params['description'] );
        }

        if($params['data_text2'])
        {
            $classAttribute->setAttribute( 'data_text2', $params['data_text2'] );
        }
