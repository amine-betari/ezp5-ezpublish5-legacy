...
{foreach $functions as $_function}
    <Policy module="{$module}" function="{$_function}">
        {if first_set($class, $section, $parent_class, $owner, $node_id, $siteaccess, $subtree, $function_list, $state, $newstate, $stategroup, false())}
        <Limitations>
            {if is_set($node_id)}
                {if is_array($node_id)}
            ...
            {if is_set($state) and is_set($stategroup)}
                {if is_array($state)}
