...
{
	$errors[] = ezpI18n::tr( 'extension/noveniniupdate/error', 'XML content does not exist');
}

// XML readable?
if ( !$xmlContent = file_get_contents( $xmlContentPath ) )
{
	$errors[] = ezpI18n::tr( 'extension/noveniniupdate/error', 'XML content is not readable');
}
	...
	{			
	    ...
	    foreach ( $lines as $line )
	    {
            ...
            if ( $line->getAttribute('env') == $selectedEnvironment )
            {
                ...
                {
                    $errors[] = ezpI18n::tr( 'extension/noveniniupdate/error', 'Invalid INI datatype:'.' '.$line->getAttribute('type'));
                }
            }
        }
        $i++;
