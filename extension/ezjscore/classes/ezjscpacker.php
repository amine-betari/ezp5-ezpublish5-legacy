...
class ezjscPacker
{
    ...
    {
        eZLog::debug($cssFiles, 'buildStylesheetTag call', 'ezjscore.log' );
        $ret = '';
        $packedFiles = ezjscPacker::packFiles( $cssFiles, 'stylesheets/', '.css', $packLevel, $indexDirInCacheHash, '_' . $media );
        eZLog::debug($packedFiles, 'buildStylesheetTag packed Files', 'ezjscore.log' );
        $http = eZHTTPTool::instance();
        $useFullUrl = ( isset( $http->UseFullUrl ) && $http->UseFullUrl );
        ...
        if ( empty($packedFiles) )
        {
          ...
          $ret = "<!-- NO CSS - please see log for more details - Timestamp: $time  -->";
          return $ret;
        }
        foreach ( $packedFiles as $packedFile )
        {
    ...
    {
        eZLog::debug($cssFiles, 'buildStylesheetFiles call', 'ezjscore.log' );
    ...
    static function packFiles( $fileArray, $subPath = '', $fileExtension = '.js', $packLevel = 2, $indexDirInCacheHash = false, $filePostName = '' )
    {
        ...
        if ( $isCSS ) eZLog::debug($fileArray, 'packFiles() call', 'ezjscore.log' );
        if ( !$fileArray )
        {
            return array();
        }
        ...
        while ( !empty( $fileArray ) )
        {
            ...
            $logcount++;
            if ( $isCSS ) eZLog::debug($file, "packFiles() iteration $logcount", 'ezjscore.log' );
            ...
            if ( $isCSS )
            {
              eZLog::write("file path : $file", 'ezjscore.log' );
              eZLog::write("file time : $fileTime", 'ezjscore.log' );
              eZLog::write("WWW file : $wwwFile", 'ezjscore.log' );
            }

            if ( $fileTime === false )
            {
                if ( $isCSS ) eZLog::write("Could not get modified time of file: $file", 'ezjscore.log' );
        ...
        if ( $isCSS ) eZLog::debug($data, "packFiles() -- data Array", 'ezjscore.log' );
        ...
        if ( $data['pack_level'] === 0 )
        {
            ...
            if ( $isCSS )
            {
              eZLog::write("Packer disabled", 'ezjscore.log' );
              eZLog::debug(array_merge( $data['http'], $data['www'] ), "packFiles() Return", 'ezjscore.log' );
            }
        ...
        else if ( empty($data['locale']) && !empty($data['http']) )
        {
            ...
            if ( $isCSS )
            {
              eZLog::write("No locale CSS, but http", 'ezjscore.log' );
              eZLog::debug(array_merge( $data['http'], $data['www'] ), "packFiles() Return", 'ezjscore.log' );
            }
        ...
        {
            if ( $isCSS ) eZLog::write("Could not find any files !", 'ezjscore.log' );
        ...
        {
            if ( $isCSS ) eZLog::write("AppendLastModifiedTime", 'ezjscore.log' );
            ...
            {
                if ( $isCSS ) eZLog::write("File found : ".$data['cache_path'], 'ezjscore.log' );
                $data['http'][] = $data['custom_host'] . $data['www_dir'] . $data['cache_path'];
                self::$log[] = $data;
                if ( $isCSS ) eZLog::debug($data['http'], "packFiles() Return", 'ezjscore.log' );
                return $data['http'];
            }
        }
        else
        {
            if ( $isCSS ) eZLog::write("NOT AppendLastModifiedTime", 'ezjscore.log' );
        ...
        if ( $isCSS ) eZLog::debug($data, "Merge file content and create new cache file", 'ezjscore.log' );
        ...
        {
            if ( $isCSS ) eZLog::write('pack_level : '.$data['pack_level'], 'ezjscore.log' );
            foreach( $ezjscINI->variable( 'eZJSCore', $isCSS ? 'CssOptimizer' : 'JavaScriptOptimizer' ) as $optimizer )
            {
                if ( $isCSS ) eZLog::write('Optimizer : '.$optimizer, 'ezjscore.log' );
        ...
        if ( $isCSS ) eZLog::debug($data, 'Data before end of packFiles()', 'ezjscore.log' );
