...
class ezjscPackerTemplateFunctions
{
    ...
    function modify( eZTemplate $tpl, $operatorName, array $operatorParameters, $rootNamespace, $currentNamespace, &$operatorValue, array $namedParameters )
    {
        ...
        switch ( $operatorName )
        {
            ...
            case 'ezcss_load':
            {
                eZLog::write( 'ezcss_load call', 'ezjscore.log' );
                if ( !isset( self::$loaded['css_files'] ) )
                {
                    ...
                    eZLog::debug($depend, 'ezcss_load depend', 'ezjscore.log' );
                    $ret = ezjscPacker::buildStylesheetTag( $depend,
                                                         $namedParameters['media'],
                                                         $namedParameters['type'],
                                                         $namedParameters['rel'],
                                                         $namedParameters['pack_level'] );
                    eZLog::debug($ret, 'ezcss_load ret', 'ezjscore.log' );
                ...
                elseif ( $operatorName === 'ezcss_load' )
                {
                    ...
                    eZLog::debug($namedParameters['css_array'], 'ezcss_load CSS Array', 'ezjscore.log' );
                }
            }
            case 'ezcss_require':
            {
                eZLog::write( 'ezcss_require call', 'ezjscore.log' );
                if ( !isset( self::$loaded['css_files'] ) )
                {
                    eZLog::write( 'ezcss_require call (first)', 'ezjscore.log' );
                ...
                {
                    $namedParameters['css_array'] = self::setPersistentArray( 'css_files', self::flattenArray( $namedParameters['css_array'] ), $tpl, true, true, true );
                    eZLog::debug($namedParameters['css_array'], 'ezcss_require CSS Array', 'ezjscore.log' );
                }
            }
            case 'ezcss':
            {
                eZLog::write( 'ezcss call', 'ezjscore.log' );
                eZLog::debug($namedParameters, 'ezcss NamedParameters', 'ezjscore.log' );
                $ret = ezjscPacker::buildStylesheetTag( $namedParameters['css_array'],
                                                     $namedParameters['media'],
                                                     $namedParameters['type'],
                                                     $namedParameters['rel'],
                                                     $namedParameters['pack_level'] );
                eZLog::debug($ret, 'ezcss ret', 'ezjscore.log' );
            } break;
            case 'ezcssfiles':
            {
                if ( $namedParameters['ignore_loaded'] )
